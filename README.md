...
- std::thread — переменная, объявленная со спецификатором thread_local (также может использоваться вместе с static или extern). Для каждого потока создаётся свой экземпляр переменной, который существует до завершения потока.
