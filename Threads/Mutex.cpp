...
/*
 std::mutex — механизм синхронизации и потокобезопасности (thread safety), который предназначен для контроля доступа к общим данным для нескольких потоков с использованием барьеров памяти. std::mutex - дорогостоящая операция, требует задействование планировщика с переводом потока в другое состояние и добавлением его в список потоков, ожидающих разблокировки.
